...
gtag('js', new Date());
gtag('config', 'AW-766229134');

window.trekkie = {
  load: function (config) {
    if (config && config['Google Gtag Pixel']) {
      // Block Trekkie
      delete config['Google Gtag Pixel'];
      console.log('Blocked Gtag Pixel configuration for trekkie.');
    }
  },
};
...
(function () {
  ...
  theme.utils = {
    ...
    serialize: function (form) {
      ...
      // Flag to check if quantity field has been added
      ...
      Array.prototype.slice.call(form.elements).forEach(function (field) {
        ...
        if (field.type === 'text' && field.name === 'quantity') {
      ...
      if (!quantityAdded) {
        var defaultQuantity = document.querySelector(
          '.js-qty__wrapper input[name="quantity"]'
        ).value;
        arr.push(encodeURIComponent('quantity') + '=' + encodeURIComponent(defaultQuantity));
      }
  ...
  theme.Variants = (function () {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _updateMasterSelect: function (variant) {
        ...
        var elements = this.container.querySelectorAll(this.originalSelectorId);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      updateSuccess: function (cart, key) {
        if (cart.total_price > 5000) {
          $('.shippingtextchange').text('Free');
        } else {
          $('.shippingtextchange').text('Calculated at Checkout');
        }
      ...
      updateCount: function (count) {
        ...
        if (count > 0) {
          ...
          document
            .querySelector(selectors.cartBubble)
      ...
      updateRow: function (key) {
        ...
        theme.cart.getCartProductMarkup().then(
          function (html) {
            var markup = this._parseProductHTML(html);
            var items = markup.items;
            var matchingPrice = items.querySelector(
              '.cart__item[data-key="' + key + '"] .cart__item-price-col'
            );
            if (matchingPrice) {
  ...
  theme.headerNav = (function () {
    ...
    function stickyHeaderInitialPosition(header) {
      ...
      if (
        parentNextSibling &&
        parentNextSibling.classList.contains('shopify-section-group-header-group')
      ) {
  ...
  theme.Collection = (function () {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      renderActiveTag: function (parent, el) {
        const textEl = parent.querySelector('a');
  ...
  theme.Product = (function () {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      initAjaxProductForm: function () {
        ...
        var addToCartForms = document.querySelectorAll('form');
      ...
      updateColorName: function (color, index) {
        ...
        const variantColorLabel = this.container.querySelector(
          '#VariantColorLabel-' + this.sectionId + '-' + index
        );

        if (variantColorLabel) {
          variantColorLabel.textContent = color;
        }
      },

      updateCartButton: function (evt) {
        var variant = evt.detail.variant;
        var cartBtns = this.container.querySelectorAll(this.selectors.addToCart);
        var cartBtnTextElements = this.container.querySelectorAll(this.selectors.addToCartText);

        if (!cartBtns.length) return;

        cartBtns.forEach((cartBtn, index) => {
          var cartBtnText = cartBtnTextElements[index];
      ...
      toggleInventoryQuantity: function (state = undefined, quantity) {
        ...
        if (parseInt(quantity) > 0 || (state && state === 'visible')) {
          if (showLowInventoryMessage) {
            ...
            if (quantity > 1) {
              productInventoryEl.textContent = theme.strings.otherStockLabel.replace(
                '[count]',
                quantity
              );
            } else {
              productInventoryEl.textContent = theme.strings.oneStockLabel.replace(
                '[count]',
                quantity
              );
            }
